from name_input import name_login          
import pygame
...
player_images = []
for i in range(1,5):
    player_images.append(pygame.transform.scale(pygame.image.load(f'assets/player_images/DI_{i}.png'), (45, 45)))    
player_x = 450
player_y = 663
name = []


blinky_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/1y.png'), (45, 45))
pinky_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/2j.jpeg'), (45, 45))
inky_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/3l.jpeg'), (45, 45))
clyde_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/4d.jpeg'), (45, 45))
spooked_img = pygame.transform.scale(pygame.image.load(f'assets/ghost_images/spooked.png'), (45, 45))
...
def eating_score(score_p, power, power_count, eaten_ghosts):
    num1 = (SCREENHEIGHT - 50) // 32
    num2 = SCREENWIDTH // 30
    if 0 < player_x < 870:
    ...
    return score_p, power, power_count, eaten_ghosts
...
def draw_other():
    ...
    if powerup:
        pygame.draw.circle(screen, 'green', (140, 930), 7 )
    ...
    if game_over:
        ...
        name_player_text = font.render(name_player, True, 'red')
        screen.blit(gameover_text, (100, 300))
        screen.blit(name_player_text, (100, 250))
    if game_won:
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300],0, 10)
        pygame.draw.rect(screen, 'dark gray', [70, 220, 760, 260], 0, 10)
...
while run:
    ...
    score, powerup, power_counter, eaten_ghost = eating_score(score, powerup, power_counter, eaten_ghost)
...
connection = psycopg2.connect(
    database="pacman_score", #your database name
    user='postgres',
...
wish_data = json.loads(response.text)
        ...
        try:
            with connection:
                with connection.cursor() as curs:
                    query = f"""
                    INSERT INTO pacman (name, score, ChuckNorris_wish)
                    Values ('{name_player}', {score_final}, '{wish_data['value']}' )
                    """
                    curs.execute(query)
                    connection.commit()
